...
import type { ContainsCompareData } from "./types";
...
export function ContainsCompareBuilder({ templateName }: Props) {
  const [data, setData] = useState<ContainsCompareData>(() => getExample());

  const [containsIds, setContainsIds] = useState<string[]>(() =>
    getExample().containsItems.map(() => crypto.randomUUID())
  );
  const [notContainsIds, setNotContainsIds] = useState<string[]>(() =>
    getExample().notContainsItems.map(() => crypto.randomUUID())
  );
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      <section className="rounded-2xl border border-black/10 bg-white p-5">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2"></div>
