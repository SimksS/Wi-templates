...
import type { ComparisonRow, ComparisonTableData } from "./types";

type Props = { templateName: string };

export function ComparisonTableBuilder({ templateName }: Props) {
  const [data, setData] = useState<ComparisonTableData>(() => getExample());

  const [rowIds, setRowIds] = useState<string[]>(() =>
    getExample().rows.map(() => crypto.randomUUID())
  );
      ...
      <section className="rounded-2xl border border-black/10 bg-white p-5">
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          <label className="rounded-xl border border-black/10 bg-zinc-50 p-3">
            ...
            <input
              ...
              value={data.subtitle}
              onChange={(e) =>
                setData((d) => ({ ...d, subtitle: e.target.value }))
              }
              className="mt-2 h-9 w-full rounded-lg border border-black/10 bg-white px-3 text-sm"
            />
          </label>
        ...
        <div className="mt-3 grid grid-cols-1 gap-3">
          ...
          <ImageUrlField
            ...
            invalid={compInvalid}
            onChange={(v) =>
              setData((d) => ({ ...d, competitorImageUrl: v }))
            }
          />
        </div>
              ...
              >
                ...
                <label className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    value={row.attribute}
                    onChange={(e) =>
                      updateRow(setData, idx, { attribute: e.target.value })
                    }
                    className="h-9 rounded-lg border border-black/10 bg-white px-3 text-sm"
                  />
                </label>
                ...
                <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  <ValueEditor
                    ...
                    onChange={(v) =>
                      updateRow(setData, idx, { competitor: v })
                    }
                  />
                </div>
              </div>
      ...
      <div className="mb-2 flex items-center justify-between gap-2">
        ...
        <select
          ...
          onChange={(e) => {
            ...
            onChange(
              kind === "text" ? { kind: "text", value: "" } : { kind: "boolean", value: true }
            );
          }}
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={[
              ...
              value.value
                ? "bg-black text-white"
                : "bg-white text-zinc-700 border border-black/10",
            ].join(" ")}
          >
            Sim
          </button>
