...
export function AttributesStripBuilder({ templateName }: Props) {
  const [data, setData] = useState<AttributesStripData>(() => {
    const ex = getAttributesStripExample();
    return ex;
  });

  const [attributeIds, setAttributeIds] = useState<string[]>(() =>
    getAttributesStripExample().attributes.map(() => crypto.randomUUID())
  );
  ...
  const rendered = useMemo(
    () => ({
      ...
      attributes: data.attributes.map((a, idx) => {
        ...
        return { ...a, iconUrl: getAttributesStripExample().attributes[0]?.iconUrl ?? "" };
      }),
    }),
  ...
  const renderedTemplate = useMemo(
    () => renderAttributesStrip(rendered),
    [rendered]
  );
...
function isDisallowedIconUrl(url: string) {
  ...
  // Se não parece ter uma extensão de imagem permitida, marcamos como inválido.
  // (Mantém o app simples e orientado ao usuário não-técnico.)
